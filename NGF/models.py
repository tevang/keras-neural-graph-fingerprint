...
def get_ngf(SMILES, model_type='1', conv_width=8, fp_length=186):
    """
    ...
    :param SMILES: list of SMILES strings of the molecules whose NGF fingerprint you wish to calculate
    :param model_type: could be '1' or '2'. In '1' the outputs of the 3 (convoluted) atom feature layers are summed, while
                        in '2' they are concatenated. Default is '1' and in terms of performance in ML is it better than '2'.
    :param conv_width: the width of each convolutional layer
    :param fp_length: the desired length of the final fingerprint vector
    :return: the final NGF vector of floats
    """
...
def build_default_model(SMILES, model_type='1', conv_width=8, fp_length=186):
    """
    ...
    if model_type == '1':
        layer_size = fp_length
        model = build_graph_conv_model(max_atoms, max_degree, num_atom_features, num_bond_features,
                                       learning_type='regression', conv_layer_sizes=[conv_width, conv_width],
                                       fp_layer_size=[layer_size, layer_size, layer_size],
                                       conv_activation='relu', fp_activation='softmax',
                                       conv_bias=False, fp_merge_mode='add')
    elif model_type == '2':
        layer_size = fp_length // 3
        model = build_graph_conv_model(max_atoms, max_degree, num_atom_features, num_bond_features,
                                       learning_type='regression', conv_layer_sizes=[conv_width, conv_width],
                                       fp_layer_size=[layer_size, layer_size, layer_size],
                                       conv_activation='relu', fp_activation='softmax',
                                       conv_bias=False, fp_merge_mode='concat')
    return model
...
def build_graph_conv_model(max_atoms, max_degree, num_atom_features,
    ...
    net_output = build_graph_conv_net([atoms, bonds, edges], fp_merge_mode=fp_merge_mode, **kwargs)
...
def build_graph_conv_net(data_input,
                         ...
                         fp_merge_mode='concat', atomwise_dropout=True,
                         graphpool=False):
    ...
    if len(fingerprint_outputs) > 1:
        ...
        if fp_merge_mode == "add":
            final_fp = merge.Add()(fingerprint_outputs)
        elif fp_merge_mode == "concat":
            final_fp = merge.Concatenate()(fingerprint_outputs)
    else:
